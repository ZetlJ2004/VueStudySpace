<template>
  <div>
    <h1>Vue Examples</h1>
    <h3><router-link to="/">Home</router-link></h3>
    <h3><router-link to="/exa01-1">Router</router-link></h3>
    <h3>Composition API</h3>
    <ul>
      <li>
        <router-link to="/exa02-1">
          interface & ref() & computed() & watch() & watchEffect()
        </router-link>
      </li>
      <li>
        <router-link to="/exa02-2">computed() & watch()-02</router-link>
      </li>
      <li>
        <router-link to="/exa02-3">reactive()</router-link>
      </li>
      <li>
        <router-link to="/exa02-4">shallowRef()</router-link>
      </li>
    </ul>
    <h3>Route With Params</h3>
    <ul>
      <li>
        <router-link to="/exa03-1/students/45/homeworks/23">
          useRoute(): students/45/homeworks/23
        </router-link>
      </li>
      <li>
        <router-link to="/exa03-1/students/18/homeworks/18">
          useRoute(): students/18/homeworks/18
        </router-link>
      </li>
    </ul>
    <h3>Directives</h3>
    <ul>
      <li>
        <router-link to="/exa05-1">template & if & v-bind</router-link>
      </li>
      <li>
        <router-link to="/exa05-2">v-bind</router-link>
      </li>
      <li><router-link to="/exa05-3">v-for</router-link></li>
    </ul>
    <h3>Form Input Bindings</h3>
    <ul>
      <li><router-link to="/exa07-1">v-model</router-link></li>
      <li><router-link to="/exa07-2">Examples</router-link></li>
    </ul>
    <h3>Component</h3>
    <ul>
      <li><router-link to="/exa06-1">Async Component</router-link></li>
      <li><router-link to="/exa06-2">keep-alive</router-link></li>
    </ul>
    <h3>Global Store</h3>
    <ul>
      <li>
        <router-link to="/exa08-1">State</router-link>
      </li>
      <li>
        <router-link to="/exa08-2">Actions</router-link>
      </li>
      <li>
        <router-link to="/exa08-3">Getters</router-link>
      </li>
      <li>
        <router-link to="/exa08-4">Define Custom Global Store</router-link>
      </li>
    </ul>
    <h3>Passing Data to Child Component</h3>
    <ul>
      <li>
        <router-link to="/exa09-1">defineProps()</router-link>
      </li>
      <li>
        <router-link to="/exa09-4">Passing Functions</router-link>
      </li>
      <li>
        <router-link to="/exa09-5">defineModel()</router-link>
      </li>
      <li>
        <router-link to="/exa09-3">defineExpose()</router-link>
      </li>
      <li>
        <router-link to="/exa09-2">defineEmits()</router-link>
      </li>
    </ul>
    <h3>Network</h3>
    <ul>
      <li>
        <router-link to="/exa10-1">VueUse useFetch</router-link>
      </li>
      <li>
        <router-link to="/exa11-1">Mock & mriage</router-link>
      </li>
      <li>
        <router-link to="/exa11-2">Login</router-link>
      </li>
      <li>
        <router-link to="/exa11-3">URL passing json params</router-link>
      </li>
    </ul>
    <h3>Nested Routes</h3>
    <ul>
      <li>
        <router-link to="/exa12-1">Nested Routes</router-link>
      </li>
    </ul>
    <h3>Navigation Guards</h3>
    <ul>
      <li>
        <router-link to="/exa13-1/login">Navigation Guards</router-link>
      </li>
    </ul>
    <h3>Loading Components</h3>
    <ul>
      <li>
        <router-link to="/exa15-1">suspense</router-link>
      </li>
      <li>
        <router-link to="/exa15-2">component/v-if</router-link>
      </li>
    </ul>
    <h3>Slots</h3>
    <ul>
      <li>
        <router-link to="/exa16">Slots</router-link>
      </li>
    </ul>
    <h3>Functional Components</h3>
    <ul>
      <li>
        <router-link to="/exa17">Functional Components</router-link>
      </li>
    </ul>
    <h3>Decorators</h3>
    <ul>
      <li>
        <router-link to="/exa18">Decorators</router-link>
      </li>
    </ul>
    <h3>Proxy</h3>
    <ul>
      <li>
        <a href="/vue-proxy.html" target="_blank">JS+HTML+Proxy</a>
      </li>
    </ul>
    <h3>Experiments & Homwork</h3>
    <ul>
      <li>
        <router-link to="/exp02">Experiment02 - Vue双向绑定实验</router-link>
      </li>
      <li>
        <router-link to="/exp03">Experiment03 - 单一数据源实验</router-link>
      </li>
      <li>
        <router-link to="/homework01">Homework - 组件动态切换</router-link>
      </li>
      <li>
        <router-link to="/homework04">Homework - Checkbox双向互交绑定</router-link>
      </li>
      <li>
        <router-link to="/homework02">Homework - 组件封装与暴露属性</router-link>
      </li>
      <li>
        <router-link to="/homework03">Homework - 基于Slot的组件内容扩展</router-link>
      </li>
      <li>
        <router-link to="/homework05">Homework - 函数式全屏Loading遮罩组件</router-link>
      </li>
    </ul>
  </div>
</template>
<style scoped>
* {
  margin: 0;
}
</style>
